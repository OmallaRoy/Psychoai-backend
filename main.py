...
# v1.2: Dual RAG — Tavily web search integrated via coaching.py
#       ChatRequest accepts last_pattern for better search queries
# ================================================================
...
# ── FastAPI app ────────────────────────────────────────────────
app = FastAPI(title="Psychoai API", version="1.2.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins  = ["*"],
    allow_methods  = ["*"],
...
class ChatRequest(BaseModel):
    trader_id:    str
    message:      str
    # Full conversation history from Android client
    # Fixes stateless conversation bug (v1.1)
    messages:     List[Dict[str, str]] = []
    # Last detected pattern — helps build better Tavily search queries
    # e.g. "Revenge Trading" guides the web search toward relevant content
    # Optional — defaults to empty string for backwards compatibility
    last_pattern: str = ""
...
async def coaching_background_task(
    ...
    """
    Thread 2 — async coaching pipeline.
    v1.2: generate_coaching now fetches Tavily web context
    automatically for the detected pattern before calling Groq.
    """
    try:
        trader_memory = load_trader_memory(trader_id)

        # generate_coaching handles Tavily search internally
        coaching = generate_coaching(
            trader_id, pred_label, confidence,
            feature_signals, retrieved_cases, trader_memory
        )
...
async def chat_with_plutus(req: ChatRequest):
    """
    v1.1: Passes full conversation history to fix stateless bug.
    v1.2: Smart web search — Tavily called only when user message
    ...
    try:
        ...
        response = generate_chat_response(
            ...
            last_pattern = req.last_pattern,  # v1.2: for Tavily query
        )
...
async def startup():
    start_scheduler()
    from config import TAVILY_API_KEY
    log.info("Psychoai backend v1.2.0 started on %s", device)
