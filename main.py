...
# v1.2: Dual RAG — Tavily web search integrated
# v1.3: Mandatory 3-step coaching prompt
# v1.4: History query fix — removed order_by from chat_sessions
#        query. Firestore requires a composite index for
#        .where() + .order_by() on different fields. Without the
#        index the query throws an error that was caught silently,
#        causing the history drawer to show empty. Sort is now
#        done in Python after fetching. No index required.
# ================================================================
...
# ── FastAPI app ────────────────────────────────────────────────
app = FastAPI(title="Psychoai API", version="1.4.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins  = ["*"],
    allow_methods  = ["*"],
...
async def get_full_history(trader_id: str):
    ...
    v1.4 FIX: Removed order_by from chat_sessions query.
    Firestore requires a composite index for .where() + .order_by()
    on different fields. Without that index the query throws an
    exception caught silently in the except block, meaning the
    chat sessions list was always empty. Sorting is now done in
    Python after fetching — no composite index needed.
    """
    try:
        import datetime
        items = []
        ...
        # ── Chat sessions ──────────────────────────────────────
        # FIX: query WITHOUT order_by — sort in Python instead.
        # order_by("timestamp") after where("trader_id") needs
        # a composite Firestore index that was never created.
        # Removing order_by makes this a simple collection scan
        # which works without any index configuration.
        try:
            chats = (
                db.collection("chat_sessions")
                  .where("trader_id", "==", trader_id)
                  .limit(50)   # fetch more, sort + trim in Python
                  .stream()
            )
            for chat in chats:
                d = chat.to_dict()
        ...
        # Sort all items by timestamp descending in Python
...
async def chat_with_plutus(req: ChatRequest):
    try:
        ...
        # Save to root chat_sessions collection with trader_id field
        # so the history endpoint can find it via the where() query.
        # NOTE: Android ChatViewModel must NOT also save to Firestore
        # as that creates a duplicate at a different subcollection path
        # that the history endpoint cannot query.
...
async def startup():
    start_scheduler()
    from config import TAVILY_API_KEY
    log.info("Psychoai backend v1.4.0 started on %s", device)
    log.info("Web RAG (Tavily): %s",
             "ENABLED" if TAVILY_API_KEY else "DISABLED (no TAVILY_API_KEY)")
    log.info("Topic restriction: ENABLED — off-topic queries redirected")
    log.info("History query fix: ENABLED — no composite index required")